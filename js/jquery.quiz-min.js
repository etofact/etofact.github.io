	...
	a.quiz = function (b, d) {
		...
		e.methods = {
			...
			setup: function () {
				...
				}), b += "</div>", 0 === a(k).length && (b += '<div id="' + k.substr(1) + '">', b += '<p id="quiz-results"></p>', b += "</div>"), b += '<div id="quiz-controls">', b += '<p id="quiz-response"></p>', b += '<div id="quiz-buttons">', b += '<a href="#" id="quiz-next-btn">Дальше</a>', b += '<a href="#" id="quiz-finish-btn">Показать результат</a>', b += '<a href="#" id="quiz-restart-btn">Пройти еше раз</a>', b += "</div>", b += "</div>", e.$el.append(b).addClass("quiz-container quiz-start-state"), a("#quiz-counter").hide(), a(".question-container").hide(), a(l).hide(), a(k).hide(), a("#quiz-controls").hide()
			},
			start: function () {
